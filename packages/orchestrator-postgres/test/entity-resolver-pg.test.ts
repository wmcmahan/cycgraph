...
describe.skipIf(!isDatabaseAvailable())('EntityResolver × DrizzleMemoryStore', () => {
  ...
  it('merges duplicates, resyncs the entity_facts join table, and rewrites edges', async () => {
  ...
  it('drops self-loops and duplicate edges against existing canonical edges', async () => {
    ...
    const selfLoop = makeRel(alice1.id, alice2.id);
    const existing = makeRel(alice1.id, acme.id);
    const duplicate = makeRel(alice2.id, acme.id);
  ...
  it('is idempotent — a second resolve pass is a no-op against the DB', async () => {
