...
- 9d396ec: Run lineage: `workflow_runs` records where a run came from.
...
- 3c2524f: Report a missing run record instead of a raw constraint failure.
...
- e35b1ab: Implement the new optional memory interfaces from `@cycgraph/memory`:
...
- 8c0ed4b: Fenced-worker tenant scoping, checkpoint fencing, and durable-queue hardening.
