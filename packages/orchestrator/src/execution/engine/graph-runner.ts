 ...
 *
 * @module execution/engine/graph-runner
 */

import { EventEmitter } from 'events';
import type { Graph, GraphNode, GraphEdge } from '../../graph/graph.js';
import type { WorkflowState, Action } from '../../state/state.js';
import { rootReducer, internalReducer, validateAction, REPLAY_VERSION } from '../../state/reducers.js';
...
import { NoopEventLogWriter } from '../../persistence/event-log.js';
...
import { evaluateSecurityPolicy } from '../../security/security-policy.js';
import { computeHumanResponseOutcome, type HumanResponse } from '../coordination/hitl-resume.js';

// Re-export for backward compatibility — canonical definition in hitl-resume.ts
export type { HumanResponse } from '../coordination/hitl-resume.js';

// Extracted modules
import { NodeExecutionDriver } from '../engine/node-execution-driver.js';
import type { NodeExecutorContext } from '../nodes/context.js';
...
export class GraphRunner extends EventEmitter {
   ...
   * arbitration, failed-attempt usage. See `execution/engine/node-execution-driver.ts`.
   */
  ...
  /** Budget threshold tracker. See `cost/budget-monitor.ts`. */
  private readonly budget: BudgetMonitor;
  /** Persistence pipeline + auto-compaction. See `execution/coordination/persistence-coordinator.ts`. */
  ...
  private async *executeLoop(): AsyncGenerator<StreamEvent> {
    ...
    try {
      while (shouldContinue(this.state) && !this.abortController.signal.aborted) {
        ...
        // events; throws on breach. See cost/execution-accounting.ts.
  ...
  // lives in NodeExecutionDriver — see execution/engine/node-execution-driver.ts.
  // Cost tracking lives in BudgetMonitor — see cost/budget-monitor.ts.
  ...
  static async recover(
    ...
    const { recoverGraphRunner } = await import('../coordination/recover.js');
