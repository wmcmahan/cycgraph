 ...
 *
 * @module execution/coordination/worker
 */

import { EventEmitter } from 'events';
import { GraphRunner } from '../engine/graph-runner.js';
import type { GraphRunnerOptions, HumanResponse } from '../engine/graph-runner.js';
  ...
  private async processJob(job: WorkflowJob): Promise<void> {
    try {
      ...
      if (latestSeqId >= 0) {
        ...
        const { WorkflowStateSchema } = await import('../../state/state.js');
