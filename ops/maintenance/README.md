...
| `CYCGRAPH_MODEL_HIGH` / `CYCGRAPH_MODEL_MEDIUM` / `CYCGRAPH_MODEL_LOW` | Per-tier model overrides. High runs the code-editing and judging agents, medium the reviewers and synthesizers, low the distillers. Each tier falls back to `CYCGRAPH_MODEL`, and every tier model must belong to the same provider. The CI workflows default medium and low to `claude-sonnet-5`, but only while `CYCGRAPH_MODEL` itself is unset: overriding the base model means owning the tiers, since a Sonnet default beside a non-Anthropic base model would fail the same-provider check at startup. The patrol workflow sets no tiers on purpose |
