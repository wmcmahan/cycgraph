/**
 * repo_checks — the gate's check run beyond the acceptance criteria.
 *
 * Off in the product (`runLocalChecks` false) and when no extra checks are
 * configured: it passes without running tree code, and CI is the gate.
 *
 * @module maintenance/implement-ticket/tools/checks
 */
...
export function checksTool(c: ImplementContext) {
