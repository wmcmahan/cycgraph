 ...
 *
 * @module maintenance/optimization-propose/tools/clone
 */
...
export function cloneTool(c: OptProposeContext) {
  ...
  return tool({
    ...
    parameters: z.object({}),
    execute: async () => {
      const ws = await cloneToBranch(repoRoot, maintenanceBranch(maintenance, `opt/scan-${randomUUID().slice(0, 8)}`), { at: workspaceAt });
