...
Taint metadata is stored in a hidden registry at `memory._taint_registry`. This key is protected at **three** levels: the **agent executor** blocks agents from writing `_`-prefixed keys in `save_to_memory` calls; the **state view** excludes `_`-prefixed keys from agent-visible memory; and the **reducers** treat `_taint_registry` as **append-only** — merging incoming taint into the existing registry rather than replacing it, so a crafted `update_memory: { _taint_registry: {} }` cannot clear or weaken taint. The executor injects `_taint_registry` as trusted system metadata after agent-level validation, and `validateAction()` skips `_`-prefixed keys during permission checks.
