...
Support varies by model rather than by provider, and the supported set changes as models ship, so this page makes no per-model claims. The engine passes the level through and an unsupported combination is rejected by the provider API at call time. Keep to `low` through `high` for agents that may run on older or smaller models, and check the provider's own reference for the current matrix: [Anthropic's effort documentation](https://platform.claude.com/docs/en/build-with-claude/effort) and [OpenAI's reasoning documentation](https://developers.openai.com/api/docs/guides/reasoning).
